    ...
    def DataGenerator(self, start, end, 
        ...
        for index, koi in self.source_data.iterrows():
            ...
            if (download):
                ...
                # SAVE DATA
                np.save(''.join([self.config.train_folder, '/global/', koi['kepoi_name'],'_global.npy']), np.array(flux_global))
                np.save(''.join([self.config.train_folder, '/local/', koi['kepoi_name'],'_local.npy']), np.array(flux_local))
