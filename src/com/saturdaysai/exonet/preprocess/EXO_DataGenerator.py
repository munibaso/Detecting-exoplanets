...
class Generator:
    ...
    def __init__(self, binsize_global=2049, binsize_local=257, mission='Kepler'):
        self.config = Config()
        source_file = os.path.join('C:\\', 'Users', 'bisqu', 'Documents', 'AI_SATURDAYS', 'EXOPLANETAS',
                                   'exonet', 'data', 'cumulative_2022.05.10_03.34.24.csv')
    ...
    def TargetGenerator(self):
        target = pd.DataFrame(columns=['id', 'target', 'disposition', 'period', 't0', 'duration'])
        print('')
        for index, koi in self.source_data.iterrows():
            if (koi['koi_disposition'] == 'CONFIRMED'):
                ...
                target_value = 0
            target = target.append({'id': koi['kepoi_name'],
                                    'target': target_value,
    ...
    def DataGenerator(self, start, end,
                      local_size=2,
                      limit_download=16,
                      plot=False):
        ...
        for index, koi in self.source_data.iterrows():
            ...
            # SEARCH DATA
            print(koi['rowid'], '-', koi['kepoi_name'], '', koi['koi_disposition'], end='.')
            if koi['kepler_name'] is not np.NaN:
                lc_target = koi['kepler_name']
            else:
                lc_target = ''.join(['KIC ', str(koi['kepid'])])
            print(' ', lc_target, end='')
            lc_search = lk.search_lightcurve(lc_target,
                                             mission=self.lc_mission,
                                             limit=16)
            ...
            if (len(lc_search) > 0):
                try:
                    ...
                    lc_collection = lc_search.download_all(quality_bitmask='hard',
                                                           download_dir='./fits',
                                                           flux_column='pdcsap_flux')
                    download = True
                except Exception as e:
                    download = False
                    print(f'Error Downloading Lightcurve. Motivo: {e}')
            else:
                download = False
            ...
            if (download):
                ...
                # GLOBAL BIN
                flux_global, nang = self.bin_lc(lc_fold, self.binsize_global,
                                                width_to_dist_ratio=1.1,
                                                limits=(-koi['koi_period'] / 2, koi['koi_period'] / 2))

                # LOCAL BIN
                local_limit = (local_size * (koi['koi_duration'] / 24)) / koi['koi_period']
                if (local_limit < koi['koi_period'] / 50):
                    local_limit = koi['koi_period'] / 50
                lc_fold_local = lc_fold[(lc_fold.phase > -local_limit) & (lc_fold.phase < local_limit)]
                flux_local, nanl = self.bin_lc(lc_fold_local, self.binsize_local,
                                               width_to_dist_ratio=1.1,
                                               limits=(-local_limit, local_limit))
                print('NaN = (', nang, ',', nanl, end=').')

                # SAVE DATA
                np.save(''.join([self.config.train_folder, '/global/', koi['kepoi_name'], '_global.npy']),
                        np.array(flux_global))
                np.save(''.join([self.config.train_folder, '/local/', koi['kepoi_name'], '_local.npy']),
                        np.array(flux_local))
        ...
        result = []
        for i in range(len(flux_global)):
            result.append(np.append(flux_local[i].flux, flux_global[i].flux))
...
if __name__ == '__main__':
    ...
    try:
        ...
        period = ''
        duration = ''
        (script, koi_name, t0, period, duration) = (script, koi_name, t0, period, duration)
    ...
    LOCAL_SIZE = 2
    LIMIT_DOWNLOAD = 16
    generator = Generator(mission=MISSION,
                          binsize_global=BINSIZE_GLOBAL,
                          binsize_local=BINSIZE_LOCAL)

    # generator.TargetGenerator()
    generator.DataGenerator(START, END,
                            local_size=LOCAL_SIZE,
                            limit_download=LIMIT_DOWNLOAD,
                            plot=True)

    lkc = Generator()
    tensor = lkc.getKOILightKurve(koi_name, float(t0), float(period), float(duration), 2049 + 1e-9, 257 + 1e-9, 4,
                                  fold_mode='split', quarter=1)
    print(tensor)
